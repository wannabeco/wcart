...
class LogicaLogin  {
    ...
    */
    public function getLoginUsuario($data,$whereIn=array(),$idTienda="")
    {
    ...
    */
    ...
    {
        extract($post);
        //verifico que el mail lque ingresó exista
    ...
    * @date 15 de Julio de 2016
    */
    public function procesoPersonas($dataLogin,$idTienda="")
    {
        ...
        if($dataLogin['estado'] == 1)//estado
        {
            ...
            //si todo ha salido bien simplemente consulto la información de la empresa y envio al usuario al home de la plataforma
                ...
                //realizo una verificación para el tema de la app móvil
                if($idTienda != "")//si es comprador de la tienda en las app móviles
                {
                    //verifica si ya es un cliente de la tienda que solicita, si no es cliente lo inserta en la tabla de relacion persona tienda
                    $verificaSiEsClienteTienda =  $this->ci->dbGeneral->getClienteTienda(array("idPersona"=>$infoEmpresa[0]['idPersona'],"idTienda"=>$idTienda));
                    if(count($verificaSiEsClienteTienda) == 0)
                    {
                        //como no es cliente proceso a relacionarlo con la tienda
                        $insertoRelacionCliente  = $this->ci->dbGeneral->relacionoCliente(array("idPersona"=>$infoEmpresa[0]['idPersona'],"idTienda"=>$idTienda,"fechaReg"=>date("Y-m-d H:i:s")));
                    }
                }
