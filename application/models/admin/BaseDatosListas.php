...
class BaseDatosListas extends CI_Model {
    private $tableDeptos                 =   "";
    private $tableCiudad                 =   "";
    private $tableMails                  =   "";
   ...
   //copear y pegar este query las veces que sea necesario realizar seleccion de datos en las tablas de la db, recordar cambiar el nombre
    public function getListas($where="")
    {
        $this->db->select("*");
        $this->db->where($where);
        $this->db->from($this->tableListas);
        $id = $this->db->get();
        //print_r($this->db->last_query());die();
    ...
    public function registraLista($dataInserta)
    {
        $this->db->insert($this->tableListas,$dataInserta);
        //print_r($this->db->last_query());die();
        return $this->db->insert_id();
    }
   //verifica la tabla
    ...
    {
        $id = $this->db->query("show tables like '".$tabla."'");
        //$id = $this->db->get();
        //print_r($this->db->last_query());die();
        return $id->result_array();
    }  
    public function creaTabla($queryCreacion)
    {
        $id = $this->db->query($queryCreacion);
        //$id = $this->db->get();
        //print_r($this->db->last_query());die();
        return $id;
    }  
