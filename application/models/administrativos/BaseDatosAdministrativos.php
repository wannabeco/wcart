...
class BaseDatosAdministrativos extends CI_Model {
    private $tableDeptos                 =   "";
    private $tableCiudad                 =   "";
    private $tableMails                  =   "";
   ...
   //copear y pegar este query las veces que sea necesario realizar seleccion de datos en las tablas de la db, recordar cambiar el nombre
    ...
    public function getPagos($where="")
    {
        ...
        $this->db->group_by('periodoAgrup');
        $id = $this->db->get();
        //print_r($this->db->last_query());die();
        return $id->result_array();
    }
    public function getPagosNoGroup($where="")
    {
        $this->db->select("*");
    ...
    {
        $this->db->insert($this->tablePagos,$dataInserta);
        //print_r($this->db->last_query());die();
        return $this->db->insert_id();
    } 
}
