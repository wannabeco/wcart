<?php
    ...
    else if($proveedor == 'AppmesPaypal'){
        $precio = _PRECIO_PLAN_BASIC;
        $nombreTrans="Pago memebresia plan Sitio web 1 mes";
        $paypal = 1;
    }
...
<?php if($paypal == 1){ ?>
    ...
    <?php 
    //“ApiKey~merchantId~referenceCode~amount~currency”.
            $llave = md5(_PAYU_API_KEY."~"._PAYU_ID_MERCADO."~".$codigoPago."~".$precio."~COP");
        ?>
        <div class="container">
            <center>
                <form method="post" id="theForm" action="https://checkout.payulatam.com/ppp-web-gateway-payu/">
                ...
                <img src="<?php echo base_url()?>/res/img/paypal.jpg" width="100%" alt="">
                <a onclick="cerarPop()" class="btn btn-primary">CERRAR VENTANA</a>
            </center>
        </div>
</div>
